...
class CLRRunner(Runner):
    """Concrete runner for the CLR GRN inference algorithm."""

    def generateInputs(self):
        """
        Function to generate desired inputs for CLR.
        If the folder/files under self.input_dir exist,
        this function will not do anything.
        """
        # Create ExpressionData.csv file in the created input directory
        CLR_EXPRESSION_FILE = self.working_dir / "ExpressionData.csv"
        input_file = self.input_dir / self.exprData
        if not CLR_EXPRESSION_FILE.exists():
            import shutil
        ...
        self.inputPath = CLR_EXPRESSION_FILE
