...
class MCP3Runner(Runner):
    """Concrete runner for the MCP3 GRN inference algorithm."""

    def generateInputs(self):
        """
        Function to generate desired inputs for MCP3.
        If the folder/files under self.input_dir exist,
        this function will not do anything.
        """
        # Create ExpressionData.csv file in the created input directory
        MCP3_EXPRESSION_FILE = self.working_dir / "ExpressionData.csv"
        input_file = self.input_dir / self.exprData
        if not MCP3_EXPRESSION_FILE.exists():
            import shutil

            shutil.copy(
                input_file,
                MCP3_EXPRESSION_FILE,
            )

        self.inputPath = MCP3_EXPRESSION_FILE
    ...
    def run(self):
        """
        Function to run MCP3 algorithm
        """
        # TODO::
        cmdToRun = " ".join(
            [
    ...
    def parseOutput(self):
        """
        Function to parse outputs from MCP3.
        """

        # Read output
        dfx = pd.read_hdf(f"{self.outFilePrefix}_inmi.mcp3.h5")
        OutDF = (
